...
|`<`|Меньше|Определяет, верно ли, что `x` меньше `y`. Все операторы сравнения возвращают `True` или `False`[1](https://py3dev.ru/bop/operators_and_expressions/#fn:1). Обратите внимание на заглавные буквы в этих словах.|`5 < 3` даст `False`, а `3 < 5` даст `True`. Можно составлять произвольные цепочки сравнений: `3 < 5 < 7` даёт `True`.|
